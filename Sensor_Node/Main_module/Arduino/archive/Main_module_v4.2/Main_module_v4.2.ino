// Main module v4.1 (does not work properly)
// Changes from v2                                                            (tested)
//  - removed SD card functionality                                           (tested)
//  - removed multiple measurement per transmission functionality             (tested)
//  - removed out commented bits of code                                      (tested)
//  - 1272 -> 939 lines
// Changes from v4.1
//  - change variable name *MEASUREMENT_EVERY_X_MIN to *SAMPLING_INTERVAL     ()
//  - change initial SAMPLING_INTERVAL from 1 to C_SAMPLING_INTERVAL          (tested)
//  - add blink function, merge if(debug_enable=True) statements              (tested)
//  - remove readDS3231time                                                   ()
//  - 939 -> *** lines
//
// To Do:
//  - Fix RTC stuff; sampling time is currently fixed to 1 minute. Reason unclear.
//  - Use watchdog timer for wake up
//  - Implement universal message library
//  - Implement heart beat
//  - Change battery warning message to universal format
...
void loop(){
  ...
  if (first_time == true && wakeUpInterrupt_flag_RTC  == true){
    ...
    Wire.beginTransmission(DS3231_I2C_ADDRESS); // Clear interrupt flag in RTC IC: Clear A2F and A1F bits by setting them to 0
    Wire.write(0x0F); // write to register (0Fh)
    Wire.write(0b10001000);
    Wire.endTransmission();
  ...
  if (wakeUpInterrupt_flag_RTC == true && ignore_rtc_interrupt == false){
    if (debug_mode_enabled == true){
      if (digitalRead(P_DBG_ENABLE) == HIGH){       // First check if slide switch still selects debug mode
    ...
    wakeup_counter++;
    for (int i=0; i<wakeup_counter; i++){
      blink(P_L3, 500); delay(500); 
    }
    ...
    if (wakeup_counter >= SAMPLING_INTERVAL){
      ...
      if (debug_mode_enabled == true){            // If we are in debug mode we want an interrupt every 10 seconds instead of every minute.
...
bool readSensorModuleData(){
  ...
  // If sensor module type has changed, or if it is the first time we read from sensor module. Or if the debug mode was disabled we need to change the size of the memory for the payload too
  if (SMtypebyte != SMtype || debug_mode_has_been_switched_off == true){
    ...
    total_xbee_payload_size = (xbee_header_bytes + sizeofSMdata);
  ...
  for (int k = 0; k < sizeofSMdata; k++){
    ...
    (total_xbee_payload + xbee_header_bytes)[k] = inByte; // formerly: " xbee_header_bytes + (measurement_counter-1)*SMsizebyte" provides offset, for second, third etc measurements
  }
...
/*****************************
   Functions for sleep with watchdog timer
*/
/*
void sleep_new(uint16_t sampling_interval) {   
    int n = SAMPLING_INTERVAL*60/4;
    for (int i=0; i<n; i++) {
      wdt_enable(WDTO_4S);
    }
}
