...
static const uint8_t SENSORMODULE_TYPE = 0x15; // 0x15 [HEX] = 21 [DEC], formerly: 0x12 [HEX] = 18 [DEC]
static const uint8_t FRAMESTART_BYTE   = 0xAA; // Frame start byte for payload transmission to main module

//******** Manual SETTINGS *********//
float min_flow          = 4;                   // Minimum Current (No conversion to flow rate)
float max_flow          = 20;                  // Maximum Current, formerly: [l/min] Maximum flow rate output of flow meter 
...
float resistor_value    = 164.7;              // [Ohm] Value of the resistor that converts the current into a voltage 
...
void setup() {
  ...
  //Serial.write('Setup successful');         // Debuggin message (remove for use with main module)
}
